    ...
    <section className="space-y-2">
      ...
      <div className="glass-panel rounded-2xl overflow-hidden shadow-[0_8px_30px_rgba(0,0,0,0.02)]">
        ...
        <div className="px-4 pb-4 space-y-3">
          ...
          {enabled && (
            <p className="text-[11px] text-on-surface-variant">
              Uses your iPhone's default notification sound, even if you choose a different sound below — the Alarm sound
              picker only applies while WakeWise is open. Tap the notification's Begin, Snooze, or Skip actions, or open
              the app, to respond.
            </p>
          )}
        </div>
...
// WakeWise DEV — alarm wake-up sound picker. Was a single "Test alarm
// sound" button (see git history) confirming WakeWise's one bundled
// foreground chime actually plays on this device/browser; now a real
// picker across the 5 intended sounds (alarmSounds.js), keeping every
// safety property that button already had - previewActiveRef (not just
// component state) is still the real ownership flag, checked before
// ever calling stopTrack(), so this section only ever stops audio IT
// started, never an unrelated track already playing elsewhere in the
// app. Deliberately its own section, not nested inside
// MorningReminderSection above (which renders nothing on web - `if
// (!supported) return null` - since it's specifically about native
// scheduling; this foreground chime is the same in-page mechanism on
// both web and native, and the picker changes ONLY this foreground
// alarm - see alarmSounds.js's own top doc comment for the native
// background-notification limit this cannot cross).
//
