...
|---|---|---|---|---|---|---|---|---|---|
| 1 | **IB01** — Evening/Quiet Breathing interactive loop | **N/A — new production, not a remix of any existing file** (first-ever asset for this screen) | `WW_IB01_InteractiveBreathingLoop_MusicBed_v1.m4a` — audio-only AAC/`.m4a`, confirmed safe end-to-end this phase (Storage, signed-URL Edge Function, CSP, Safari/Chrome `<audio>`, no code assumes `video/mp4`); see the spec doc §3a for the full audit. No video track — deliberately not the `.mp4` container used by every narrated item below | Producer's choice; recommend **45–90s single pass** (the native `loop` attribute repeats it indefinitely regardless of length — long enough that a listener doing a 64–76s breathing cycle doesn't obviously hear the seam on every repeat) | Minimal Textural Bed (same family as the Breathing category in §2 — steady drone/pad only, no rhythmic pulse) | No narration to sit "beneath." Use a practical CapCut level as a **starting point only** (comfortably under clipping, roughly matched by ear to the other files) — **do not treat CapCut's meter as a certified LUFS-integrated/true-peak reading**, since that hasn't been verified for this tool. The real validation is listening at the app's actual fixed **0.35 (35%) playback volume** (`InteractiveBreathingMusic.jsx`'s `DEFAULT_VOLUME`) on a real device | **No fade at the file boundaries** — must loop seamlessly (tail flows into head with no audible click/seam), since the native `loop` attribute jumps instantly; a fade-to-silence at each boundary would create an audible dip on every repeat | **Intended: yes**, via the native `loop` attribute — but **not guaranteed sample-accurate/gapless** with this approach (AAC encoder priming/padding plus the HTML5 `<audio loop>` spec not guaranteeing gaplessness). Must be confirmed by ear on a **physical iPhone**, not assumed; a persistent audible gap would require a Web Audio API loop instead (separate engineering, not in scope now) | `exercises/WW_IB01_InteractiveBreathingLoop_MusicBed_v1.m4a` |
...
**On the `.m4a` (audio-only) format for `IB01` specifically:** confirmed safe this phase across every layer the asset passes through — Safari/iOS `<audio>`, Chrome, Supabase Storage, the signed-URL Edge Function's format-agnostic `Map` lookup, the CSP `media-src` directive (host-based, not MIME-based — same origin either way), and a full grep of the client codebase for any `video/mp4` assumption (none found). Full audit and reasoning: `docs/background-music-specification.md` §3a. Not yet uploaded, registered, or deployed — this is a format decision only, made ahead of production so Mamun doesn't produce the wrong container.
