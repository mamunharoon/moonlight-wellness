...
| **Format/encoding** | Match the existing pipeline exactly: encode into the same H.264/AAC `.mp4` container every other `BETA_VIDEO_MANIFEST` entry uses — no new container/codec, no separate audio-only distribution format. **Scope: this row governs the 65-id pre-mixed narrated catalogue (§2's `-music` variants), where the file already is (and stays) a video file.** `IB01` (§3a below) is a different case — a brand-new, audio-only asset with no video content of its own — and is deliberately not held to "no separate audio-only distribution format" for that reason. |
| **Licensing** | Every music bed needs a **commercial-use license appropriate for a paid subscription wellness app**: a royalty-free library license with an explicit commercial/app clause, or a custom work-for-hire with full buyout, in both cases **perpetual and worldwide** (matching the app's own AU-and-beyond distribution). No track may be used under a personal/non-commercial, attribution-only (unless attribution is contractually acceptable and tracked), or ambiguous license. Retain the license document/receipt per track — this is normal App Store/legal due diligence, not unique to WakeWise. Applies to `IB01` identically — see §3a. |

**A caution on loudness metering, applying to every row above:** do not assume any given editor's on-screen meter is a calibrated, standards-accurate LUFS-integrated or true-peak meter unless that's been separately verified for the specific tool/version in use. Treat an in-app meter reading as a rough starting point, and validate the finished mix by ear — at the volume it will actually play back at in the app — before treating loudness as correct. See §3a for how this applies concretely to `IB01`.

### 3a. `IB01` — audio-only format decision (Phase C audit)

`IB01` (the `InteractiveBreathingMusic.jsx` loop for `/evening-breathing` and `/quiet-breathing`) is the one asset in this whole catalogue with no video content at all — the player is a plain HTML `<audio>` element, nothing is ever shown on screen. An H.264/AAC `.mp4` container for it would mux a video track (even if blank) purely to match the other 65 files' container, at the cost of extra Storage size, extra decode work, and extra signed-URL/CDN egress for bytes that are never displayed.

This phase audited whether an audio-only `AAC` file in an `.m4a` container (**`WW_IB01_InteractiveBreathingLoop_MusicBed_v1.m4a`**) is safe end-to-end, checking every layer the asset actually passes through:

| Layer | Finding |
|---|---|
| **Safari / iOS `<audio>`** | Safe. AAC-in-`.m4a` is Apple's own native audio format (iTunes/Apple Music/Voice Memos all use it) — WKWebView's `<audio>` element has supported it natively for as long as iOS has shipped. No compatibility risk. |
| **Chrome (desktop/Android)** | Safe for the actual shipped Google Chrome browser, which includes proprietary codec support (AAC included) — this is what real users and this project's own browser QA use. The caveat: an unbranded open-source Chromium build without proprietary codecs can lack AAC support; not a concern for WakeWise's own QA process, but worth knowing if a teammate ever tests in plain Chromium specifically. |
| **Supabase Storage MIME type** | Not a blocker, but not automatic either — the object's stored `Content-Type` (`audio/mp4` or `audio/x-m4a`) is whatever the upload step sets (explicitly, or inferred from the `.m4a` extension by the uploading client). This needs to be set correctly at upload time — a checklist item for whoever uploads the file, not a code change. |
| **Signed-URL Edge Function (`get-beta-video-url`)** | Confirmed safe by direct code read: `EXERCISE_PATHS` is a plain `Map<string, string>` with no format/extension validation on the key or the path — `createSignedUrl()` signs whatever Storage path string is registered, `.m4a` or `.mp4` alike. Zero logic changes needed to support `.m4a` — only the eventual new map entry itself. |
| **CSP (`media-src`)** | Confirmed safe by direct read of both `index.html`'s `<meta>` CSP tag and `vercel.json`'s HTTP header — identical policy in both, `media-src 'self' blob: https://kvdxuhyndevrfvsalgnx.supabase.co`. This directive is **host-based, not MIME/extension-based** — it allows any media fetched from the Supabase project's own domain (where the signed URL points) regardless of container or codec. No CSP change needed for `.m4a`. |
| **Edge Function mapping accepting `.m4a`** | Same finding as the Edge Function row above — yes, a `.m4a` path value works identically to a `.mp4` one; nothing about the key/value pair's extension is inspected anywhere in that function. |
| **Any code assuming `video/mp4`** | Confirmed none, by grepping the client codebase for `<source type=…>`, `.endsWith('.mp4')`, and MIME-type checks. `storagePath`/the signed URL are treated as opaque strings everywhere they're used (`betaVideoManifest.js`, `betaVideoAccess.js`, `BetaVideoModal.jsx`, `InteractiveBreathingMusic.jsx`) — every one of them sets `.src` directly from the returned URL string with no `type` attribute and no extension check. |

**Conclusion: `.m4a` is confirmed safe on every layer audited.** `WW_IB01_InteractiveBreathingLoop_MusicBed_v1.m4a` is the correct target filename — see the asset-manifest doc §6 (updated) for the current filename/path.

**Corrected production guidance for `IB01` specifically:**
- **Do not treat CapCut's on-screen level/peak meter as a certified LUFS-integrated or true-peak measurement** unless that's actually been verified for the CapCut version in use — it has not been verified here. Use it only as a rough starting point (e.g. keep the waveform comfortably under clipping, roughly matching the perceived loudness of the app's other files by ear), then **validate the finished loop by listening at the app's actual fixed playback volume (0.35 / 35%, `InteractiveBreathingMusic.jsx`'s `DEFAULT_VOLUME`)** on a real device before considering the mix correct — not by trusting a meter reading in isolation.
- **Seamless looping is not guaranteed by this approach and must be confirmed on a physical iPhone, not assumed.** Two independent reasons: (1) AAC encoding can leave a small silence "gap" at the very start/end of a file from encoder priming/padding samples unless the encoder writes (and the player honours) gapless-playback metadata — not something to assume without testing: and (2) the native HTML5 `<audio loop>` attribute (`audio.loop = true`, exactly what `InteractiveBreathingMusic.jsx` uses) is not specified or guaranteed to be sample-accurate/gapless by any browser — unlike the Web Audio API's `AudioBufferSourceNode` looping, which is sample-accurate but is a materially different playback architecture this component doesn't use. **Honest limitation:** with the current native-`<audio loop>` implementation, a perfectly seamless, zero-gap loop cannot be promised in advance — it must be verified by ear, specifically in a real WKWebView build on a physical iPhone (not just desktop Chrome), once the file exists. If that test reveals an audible gap that production/encoding adjustments can't remove, switching to a sample-accurate Web Audio API loop is the fallback fix — a real engineering change, out of scope until that need is confirmed.
