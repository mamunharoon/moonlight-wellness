...
15. Watch each step in the Codemagic build log as it runs — they're
    ...
    pod install → build number → fetch signing certificate/profile →
    install certificate into keychain → apply profiles to the Xcode
    project (generates `export_options.plist`) → verify
...
generated by `xcode-project use-profiles`, which in turn only has
something to match if "Fetch signing certificate and provisioning
profile" (`app-store-connect fetch-signing-files`) and "Install the
fetched certificate into the build keychain" (`keychain
add-certificates`) already ran successfully, in that order, earlier in
the same build. If the verification step's diagnostic output shows the
certificate/profile fetch itself failed or was skipped, treat it as a
...
Connect app record and registered Apple Developer identifier.
