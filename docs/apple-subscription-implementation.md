...
- Configuring the trial/founding-offer mechanism per whichever alternative in `docs/apple-subscription-architecture.md` §6/§16 is approved (**still unresolved — see below**).
...
- The trial-vs-founding-offer commercial decision itself (see below) — this phase's code supports either outcome (the trial-gating code is provider-agnostic; the Apple product/offer configuration is entirely dashboard-side and untouched) but does not decide it.

---

## Explicit decisions still requiring approval

Carried over from `docs/apple-subscription-architecture.md` §16 (still unresolved — nothing in this phase decided them, since they are dashboard/commercial decisions, not code):
1. Trial vs. founding-offer conflict — still unresolved.
2. Family Sharing — still disabled per this task's own approved decision (no App Store Connect action was taken to touch this either way).
3. `provider_subscriptions` conflict-resolution tie-break rule — implemented as "most-recently-verified wins" in `entitlementResolution.js`, per the architecture document's own recommended default; still worth explicit confirmation before this is wired into a live read path.

New, from this phase's real implementation work:
4. **`autoAcknowledgePurchases` default** (see "Where implementation differed" above) — confirm whether to switch to manual, server-confirmed acknowledgment once real verification exists.
5. **iOS 14 support being dropped** (a direct, disclosed consequence of the plugin choice) — confirm this is acceptable; the alternative (a hand-rolled StoreKit 2 plugin supporting a lower deployment target with conditional compilation) is a materially larger engineering task not undertaken here.
6. **Cancellation-detection heuristic** — needs physical-device/sandbox confirmation before being trusted; if wrong, the user-facing symptom is a cancelled purchase showing a generic "failed" message instead of the correct neutral "cancelled" one — never an entitlement/security issue either way.
