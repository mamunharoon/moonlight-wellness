...
- The trial-vs-founding-offer commercial decision itself was **approved 2026-09-16** (see below) — this phase's code already supports the approved outcome (the trial-gating code is provider-agnostic; the Apple product/offer configuration is entirely dashboard-side and still untouched, including the approved fallback-to-trial-only path if the founding offer proves unworkable at dashboard-configuration time).

---

## Explicit decisions still requiring approval

**Update (2026-09-16): items 1, 2, 3, and 5 below are now approved by the user.** See `docs/apple-subscription-architecture.md` §6/§7/§16 for the full recorded decisions. Kept here, marked resolved rather than deleted, for the historical record.

Carried over from `docs/apple-subscription-architecture.md` §16:
1. ~~Trial vs. founding-offer conflict~~ — **RESOLVED, approved 2026-09-16**: two mutually exclusive introductory paths (standard 7-day trial on monthly or annual; founding AUD $49.99 Pay-Up-Front annual, no trial, renews at $59.99), a subscriber can never receive both, eligibility/availability determined by Apple/verified server state only — never the client. **Explicit approved fallback**: if App Store Connect cannot support the founding offer securely and consistently (still to be confirmed hands-on at dashboard-configuration time — see `docs/ios-xcode-handoff.md` §15), launch with the 7-day trial only and defer the founding offer. No code in this repository needed to change for this approval — the trial-eligibility gating this phase already built (Phase D, server-side `trial_used_at`) is provider-agnostic groundwork consistent with either outcome, and Apple-side offer configuration is entirely dashboard work not yet done.
