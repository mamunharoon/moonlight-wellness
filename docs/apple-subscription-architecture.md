...
WakeWise's current native iOS build is **not App-Review-safe today**: it lets an iOS user complete a real Stripe purchase from inside the native app with zero platform branching (§3). This must be fixed before any App Store submission (TestFlight distribution itself does not go through App Review in the same way, but Beta App Review can still reach this screen — see §3).
...
### Recommended commercial alternatives (requires explicit user approval — see §16)

1. **Founding Pay-Up-Front only, no separate Apple trial** *(this document's suggested default)* — new Apple subscribers see one introductory offer: pay AUD $49.99 up front for the first year, renewing at AUD $59.99/year thereafter. Simplest to configure (a single, StoreKit-native introductory offer, no custom server-signed promotional offer needed), matches "founding member" branding directly, and avoids the trial-then-convert funnel entirely on iOS. **Loses the 7-day trial specifically on the Apple channel.**
...
**This document does not select one of these on the user's behalf** — per the task's explicit instruction, this is flagged in §16 as a decision requiring approval before any Apple-side implementation begins.
...
- **Existing Stripe subscribers on Apple:** recognised via the unified entitlement model (§7) — they are **not** silently migrated to an Apple product; they simply keep working through Stripe until they either keep using the web or separately choose to subscribe again via Apple (at which point both provider rows exist and the higher-precedence one wins, §7).
- **Family Sharing:** enabled per-subscription in App Store Connect, cannot be turned off again once turned on, and is a distinct decision requiring explicit user approval (§16) — it multiplies effective entitlement across up to five family members per paying subscriber, a real revenue/business decision, not a technical default.
...
| Conflict resolution | A precedence function (not shown as SQL here — belongs in an Edge Function or a `SECURITY DEFINER` recompute routine) resolves `entitlements` from all of a user's `provider_subscriptions` rows: **the non-expired row with the latest `current_period_expires_at` wins**; ties resolved by provider (an explicit product decision, not a technical one — recommend "the most recently verified row wins" as the least-surprising default, pending §16) |
...
1. **Trial vs. founding-offer conflict (§6)** — pick one of the four alternatives (or propose a fifth) before any Apple product/offer is configured. This is the single most consequential open decision in this document.
2. **Family Sharing (§6, §14)** — enable or not; cannot be reversed once turned on for a subscription.
3. **Client library choice (§5)** — confirm a StoreKit-2-wrapping Capacitor plugin actually supports Capacitor 7.6.9 before committing to Option B; fall back to Option A (in-house plugin) if none does. This task did not install any dependency to test this directly, per instruction.
4. **`provider_subscriptions` conflict-resolution tie-break rule (§7)** — this document recommends "most-recently-verified row wins" as a default; confirm this is the desired business behaviour, especially for the edge case of a user with simultaneously active Stripe and Apple subscriptions.
5. **Whether to fix the two pre-existing Stripe gaps (§2, §15 step 12) as part of this work or as a separate task** — they were discovered during this audit, not newly introduced, and are not required for Apple compliance specifically.
6. **Whether to pursue the StoreKit External Purchase Link Entitlement (§3) at all** — this document does not recommend it as WakeWise's primary path, but it remains a legitimate secondary option Apple offers and was not ruled out entirely.
