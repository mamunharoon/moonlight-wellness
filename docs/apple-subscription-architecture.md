...
- **The most important open finding is commercial, not technical:** Apple's own introductory-offer rules (§6) do not allow the 7-day free trial and the AUD $49.99 founding first-year price to be given to the same new Apple subscriber as stated in the approved commercial intent. This requires an explicit decision from the user before any Apple-side implementation — see §6 and §16.
...
### The core feasibility question: can the 7-day trial and the $49.99 founding first-year price be combined?

**No — not as stated.** Apple's own documentation and developer forums consistently describe exactly three **introductory offer** types — **Free Trial**, **Pay-As-You-Go**, and **Pay-Up-Front** — and state that **each subscriber is eligible for exactly one introductory offer per subscription group, ever, and only as a first-time subscriber to that group** (a returning or existing subscriber is not eligible for a second one). A 7-day free trial and a discounted first-year "Pay-Up-Front" price are **both** introductory-offer types — they are **alternatives**, not stackable benefits, for the same new subscriber. This is corroborated identically across multiple independent Apple-authored/community sources located during this session's research (§4, §17); the canonical Apple page name is "Implementing introductory offers in your app," and the "one per subscription group, first-time-only" rule should be re-confirmed by directly reading that page before implementation, since this session's fetch tool could not render its full body text.

This directly contradicts the task's stated "Approved commercial intent," which lists both "Seven-day free trial" and "Founding first year: AUD $49.99" as if simultaneously available to a new subscriber.

### What Apple's mechanisms can and cannot do here

- **Introductory offers** (trial, pay-as-you-go, pay-up-front) — for **new, first-time** subscribers to the group only; **one per subscriber per group, ever**.
- **Promotional offers** — for **existing or previous** subscribers only, not first-time signups; more flexible (custom eligibility, custom pricing) but require WakeWise's own server to generate a signed JWS for every redemption — meaningfully more engineering than an introductory offer, which any eligible client can redeem from the standard StoreKit paywall with no server involvement.
- **Offer codes** — a distribution mechanism (a redeemable code, e.g. for marketing) layered on top of the above offer types; does not create a new offer type by itself.
...
### Recommended commercial alternatives — **RESOLVED, approved 2026-09-16**

1. **Founding Pay-Up-Front only, no separate Apple trial** *(this document's original suggested default)* — new Apple subscribers see one introductory offer: pay AUD $49.99 up front for the first year, renewing at AUD $59.99/year thereafter. Simplest to configure (a single, StoreKit-native introductory offer, no custom server-signed promotional offer needed), matches "founding member" branding directly, and avoids the trial-then-convert funnel entirely on iOS. **Loses the 7-day trial specifically on the Apple channel.**
2. **Trial only, no founding discount on Apple** — new Apple subscribers get the standard 7-day free trial at full price thereafter; the AUD $49.99 founding price is offered to early **web/Stripe** subscribers only (note: Stripe's own trial is separately unconfirmed as actually wired — §2). **Loses the founding hook specifically on the Apple channel.**
3. **Trial via introductory offer, founding price via a follow-up Promotional Offer** — technically plausible (a user who converts from a free trial to their first paid period may already count as an "existing subscriber" eligible for a promotional offer on that same first renewal) but **not independently confirmed against a primary source in this session** and meaningfully more engineering (server-signed JWS per redemption). Flag for a dedicated follow-up research task before committing to this path.
4. **No Apple-side founding offer at all** — Apple subscribers always pay standard pricing (with or without the trial); "founding member" is marketed and honoured only via Stripe/web for the launch window. Lowest engineering complexity, but breaks the "one unified WakeWise entitlement" spirit if the founding price is meant to be a cross-platform badge of early support rather than a Stripe-specific perk.

**Approved decision (2026-09-16): a fifth alternative, closest in spirit to a controlled combination of 1 and 2.** WakeWise will offer **two mutually exclusive introductory paths** on Apple, not just one blanket choice for every subscriber:

- **Standard customers**: an eligible new subscriber receives the existing 7-day free-trial introductory offer on either the monthly or annual product.
- **Founding members**: an eligible new subscriber instead receives the annual product at a AUD $49.99 Pay-Up-Front introductory offer for the first year, **no free trial**, renewing at the standard AUD $59.99 annual price thereafter unless cancelled.
- **A subscriber cannot receive both** — this is exactly what this document's own feasibility finding above already established is Apple's own rule (one introductory offer per subscription group, per subscriber, ever); the approval does not attempt to override that rule, it explicitly relies on it to keep the two paths mutually exclusive.
- **Eligibility and offer availability must be determined by Apple/verified server state, never merely by the client.** This matches the architecture already designed here: a client never asserts "I am eligible for the founding offer" — it is StoreKit's/App Store Connect's own introductory-offer eligibility determination (and, server-side, the App Store Server API's transaction/offer data — §8) that decides what a given subscriber can actually redeem. `docs/apple-subscription-implementation.md`'s Phase B UI copy ("a free trial or introductory offer may be available — the App Store will show your exact eligibility and terms") was written before this approval but already matches it exactly, precisely because it defers to Apple rather than promising anything client-side.
- **Open technical question, not yet resolved by this approval, and explicitly anticipated by the user's own fallback instruction:** whether App Store Connect can cleanly configure *two distinct, eligibility-scoped* introductory offers on the same subscription group for two different subscriber cohorts (rather than one blanket introductory offer per product) has not been confirmed against App Store Connect itself in this pass — this requires hands-on dashboard verification at implementation time (§14), not further document research. **If Apple cannot support this securely and consistently — e.g. the founding offer can't be reliably scoped, or scoping it would require trusting client-asserted eligibility — WakeWise launches with the 7-day trial only, and the founding offer is deferred**, exactly as approved. This is not a fallback this document selects on its own; it is the explicit, pre-approved contingency to apply if the dashboard-time investigation finds the founding offer cannot be done safely.

Option 3 (trial via introductory offer, founding price via a follow-up Promotional Offer to a just-converted subscriber) remains a possible alternative if the primary approach above cannot be configured cleanly, but was not the approved default and still carries the same "not independently confirmed" and "meaningfully more engineering" caveats noted above.
...
| 2 | Configure the chosen introductory offer (§6 decision, §16) | App Store Connect |
| 3 | Select price points closest to AUD $7.99/$59.99/$49.99 across territories | App Store Connect |
| 4 | Decide and configure Family Sharing (§6/§16 decision) | App Store Connect |
...
1. **Commercial decision** (§6/§16) — resolve the trial-vs-founding-offer conflict and the Family Sharing question. Nothing below can be scoped precisely until this is settled.
...
**Update (2026-09-16): items 1, 2, and 4 below are now approved** — see the "Approved (2026-09-16)" notes in §6 and §7 above for the exact decisions. Item 3 was separately resolved by real implementation evidence in `docs/apple-subscription-implementation.md` Phase A. Kept here, marked, rather than deleted, so the historical record of what was open and when it closed stays intact.

1. ~~**Trial vs. founding-offer conflict (§6)**~~ — **RESOLVED, approved 2026-09-16**: two mutually exclusive introductory paths (standard 7-day trial; founding AUD $49.99 Pay-Up-Front, no trial, renews at $59.99), Apple/server-verified eligibility only, with an explicit approved fallback to trial-only if the founding path cannot be configured securely and consistently in App Store Connect. See §6.
...
6. **Whether to pursue the StoreKit External Purchase Link Entitlement (§3) at all** — **still open**. This document does not recommend it as WakeWise's primary path, but it remains a legitimate secondary option Apple offers and was not ruled out entirely; no approval has been given either way.
7. **New (2026-09-16): `autoAcknowledgePurchases`** — approved to keep the plugin's documented default *unless* its StoreKit 2 implementation requires WakeWise-controlled acknowledgement for server verification to work correctly; the actual verified behaviour must be documented (via macOS/Xcode/sandbox testing — none of which has happened yet) before any change away from the default is made. See `docs/apple-subscription-implementation.md`'s own note on this.
